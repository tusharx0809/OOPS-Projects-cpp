...
class Parking{
    ...
    public:
        ...
        bool parkVehicle(int floor, int spot, const string& licensePlate, const string& time){
            if(floor < 0 || floor >=floors || spot < 0 || spot >=numberOfParking){
                cout<<"Invalid floor or spot!"<<endl;
                return false;
            }
            if(parkingLot[floor][spot].first != ""){
                cout<<"Parking already occupied!"<<endl;
                return false;
            }
            parkingLot[floor][spot] = {licensePlate, time};
            cout<<"Vehicle parked at floor "<<floor<<" ,spot "<<spot<<endl;
            return true;
        }
...
int main() {
    Parking parking(2, 3); // 2 floors, 3 parking spots per floor

    parking.parkVehicle(0, 0, "ABC123", "10:00AM");
    parking.parkVehicle(1, 1, "DEF456", "11:30AM");
    parking.displayParkingLot();

    parking.removeVehicle(0, 0);
    parking.displayParkingLot();
